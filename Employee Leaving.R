...
hr_train=read.csv("D:\\data ANALYTICS AND SCIENCE\\PROJECT SUBMISSION DATA\\HR\\hr_train.csv",stringsAsFactors = FALSE)
View(hr_train)
hr_test=read.csv("D:\\data ANALYTICS AND SCIENCE\\PROJECT SUBMISSION DATA\\HR\\hr_test.csv",stringsAsFactors = FALSE)
View(hr_test)
...
write.csv(test_pred,"hr_submission.csv",row.names=F)
